using Newtonsoft.Json;
using Microsoft.IdentityModel.Tokens;
using System.IO;
using System.Security.Cryptography;
using Microsoft.AspNetCore.Builder;
using System;

// derived from https://github.com/mrsheepuk/ASPNETSelfCreatedTokenAuthExample

namespace JsonWebTokenSample.Common
{
    public interface IArtifacts
    {
        void setKey(RsaSecurityKey key);
        void setSigningCredentials(SigningCredentials signingCredentials);
        void setJwtBearerOptions(JwtBearerOptions jwtBearerOptions);

        string getTokenIssuer();
        string getTokenAudience();

        string getSecretKeyFile();

        JwtBearerOptions getJwtBearerOptions();

    }

    public class KeyArtifacts : IArtifacts
    {

        protected const string _secretKeyFile = "secret.key.json";
        //protected const string _tokenIssuer = "localhost"; // TODO: Get real hostname.
        //protected const string _tokenAudience = "localhost/resources"; // TODO: Pass into constructor?

        private RsaSecurityKey _key;
        private SigningCredentials _signingCredentials;
        private JwtBearerOptions _jwtBearerOptions;

        override public string ToString()
        {
            return String.Format("issuer:{0} audience:{1}",
                    getTokenIssuer(),
                    getTokenAudience());
        }
        public void setKey(RsaSecurityKey key)
        {
            _key = key;
        }

        public void setSigningCredentials(SigningCredentials signingCredentials)
        {
            _signingCredentials = signingCredentials;
        }

        public SigningCredentials getSigningCredentials()
        {
            return _signingCredentials;
        }

        public void setJwtBearerOptions(JwtBearerOptions jwtBearerOptions)
        {
            _jwtBearerOptions = jwtBearerOptions;
        }

        public string getTokenIssuer()
        {
            return Resources.tokenIssuer; // TODO: Do this for real instead of fake.
        }

        public string getTokenAudience()
        {
            return Resources.tokenAudience; // TODO: Do this for real instead of fake.
        }

        public string getSecretKeyFile()
        {
            return _secretKeyFile;
        }

        public JwtBearerOptions getJwtBearerOptions()
        {
            if (_jwtBearerOptions == null)
                throw new System.IO.InvalidDataException("Bearer missing");

            return _jwtBearerOptions;
        }
    }


    public class RSAKeyUtils
    {


        /// <summary>
        /// Create secret.key.json in web root path unless it exists.  Then get signing credentials and bearer options into the output parameter.
        /// </summary>
        public static string InitializeKeyArtifacts(string WebRootPath, /*out:*/ IArtifacts output)
        {
            var secretFileName = WebRootPath + "\\" + output.getSecretKeyFile();

            if (!File.Exists(secretFileName))
                RSAKeyUtils.GenerateKeyAndSave(secretFileName);
            var param = RSAKeyUtils.GetKeyParameters(secretFileName);
            var key = new RsaSecurityKey(param);
            output.setKey(key);
            var signingCredentials = _CreateSigningCredentials(key);
            output.setSigningCredentials(signingCredentials);
            var jwtBearerOptions = _CreateJwtBearerOption(key, output.getTokenIssuer(), output.getTokenAudience());
            output.setJwtBearerOptions(jwtBearerOptions);

            return secretFileName;

        }

        /// <summary>
        /// Generate RSA Parameter randomly
        /// </summary>
        /// <returns></returns>
        public static RSAParameters _GetRandomKey()
        {
            return Encryption.GetRandomKey();
        }

        /// <summary>
        /// Generate and save the key info into input file in JSON format 
        /// </summary>
        /// <param name="file">the file(full path) contains key info</param>
        public static void GenerateKeyAndSave(string file)
        {
            var p = _GetRandomKey();
            RSAParametersWithPrivate t = new RSAParametersWithPrivate();
            t.SetParameters(p);
            File.WriteAllText(file, JsonConvert.SerializeObject(t));
        }

        /// <summary>
        /// Get key parameter from input file.
        /// This expects a file in the format: 
        /// {
        ///  "Modulus": "",
        ///  "Exponent": "AQAB",
        ///  "P": "+VsETS2exORYlg2CxaRMzyG60dTfHSuv0CsfmO3PFv8mcYxglGa6bUV5VGtB6Pd1HdtV/iau1WR/hYXQphCP99Pu803NZvFvVi34alTFbh0LMfZ+2iQ9toGzVfO8Qdbj7go4TWoHNzCpG4UCx/9wicVIWJsNzkppSEcXYigADMM=",
        ///  "Q": "1UCJ2WAHasiCdwJtV2Ep0VCK3Z4rVFLWg3q1v5OoOU1CkX5/QAcrr6bX6zOdHR1bDCPsH1n1E9cCMvwakgi9M4Ch0dYF5CxDKtlx+IGsZJL0gB6HhcEsHat+yXUtOAlS4YB82G1hZqiDw+Q0O8LGyu/gLDPB+bn0HmbkUC2kP50=",
        ///  "DP": "CBqvLxr2eAu73VSfFXFblbfQ7JTwk3AiDK/6HOxNuL+eLj6TvP8BvB9v7BB4WewBAHFqgBIdyI21n09UErGjHDjlIT88F8ZtCe4AjuQmboe/H2aVhN18q/vXKkn7qmAjlE78uXdiuKZ6OIzAJGPm8nNZAJg5gKTmexTka6pFJiU=",
        ///  "DQ": "ND6zhwX3yzmEfROjJh0v2ZAZ9WGiy+3fkCaoEF9kf2VmQa70DgOzuDzv+TeT7mYawEasuqGXYVzztPn+qHhrogqJmpcMqnINopnTSka6rYkzTZAtM5+35yz0yvZiNbBTFdwcuglSK4xte7iU828stNs/2JR1mXDtVeVvWhVUgCE=",
        ///  "InverseQ": "Heo0BHv685rvWreFcI5MXSy3AN0Zs0YbwAYtZZd1K/OzFdYVdOnqw+Dg3wGU9yFD7h4icJFwZUBGOZ0ww/gZX/5ZgJK35/YY/DeV+qfZmywKauUzC6+DPsrDdW1uf1eAety6/huRZTduBFTwIOlPdZ+PY49j6S38DjPFNImn0cU=",
        ///  "D": ""
        /// }
        /// </summary>
        /// <param name="file">the file(full path) contains key info</param>
        /// <returns>RSAParameters object</returns>
        public static RSAParameters GetKeyParameters(string file)
        {
            if (!File.Exists(file))
                throw new FileNotFoundException("Check configuration - cannot find authenticate key file: " + file);

            var keyParams = JsonConvert.DeserializeObject<RSAParametersWithPrivate>(File.ReadAllText(file));
            return keyParams.ToRSAParameters();
        }

        /// <summary>
        /// Create signing credential for generating access token string.
        /// </summary>
        /// <param name="key">The key to signing credentials</param>
        /// <returns>SigningCredentials object</returns>
        public static SigningCredentials _CreateSigningCredentials(RsaSecurityKey key)
        {

            return Encryption.CreateSigningCredentials(key);
            
        }

        /// <summary>
        /// Create JwtBearer Option based on input information
        /// </summary>
        /// <param name="key"></param>
        /// <param name="tokenIssuer"></param>
        /// <param name="tokenAudience"></param>
        /// <returns></returns>
        public static JwtBearerOptions _CreateJwtBearerOption(RsaSecurityKey key, string tokenIssuer, string tokenAudience)
        {
            return Encryption.CreateJwtBearerOption(key, tokenIssuer, tokenAudience);


        }

    /// <summary>
    /// Util class to allow restoring RSA parameters from JSON as the normal
    /// RSA parameters class won't restore private key info.
    /// </summary>
    private class RSAParametersWithPrivate
        {
            public byte[] D { get; set; }
            public byte[] DP { get; set; }
            public byte[] DQ { get; set; }
            public byte[] Exponent { get; set; }
            public byte[] InverseQ { get; set; }
            public byte[] Modulus { get; set; }
            public byte[] P { get; set; }
            public byte[] Q { get; set; }

            public void SetParameters(RSAParameters p)
            {
                D = p.D;
                DP = p.DP;
                DQ = p.DQ;
                Exponent = p.Exponent;
                InverseQ = p.InverseQ;
                Modulus = p.Modulus;
                P = p.P;
                Q = p.Q;
            }
            
            public RSAParameters ToRSAParameters()
            {
                return new RSAParameters()
                {
                    D = this.D,
                    DP = this.DP,
                    DQ = this.DQ,
                    Exponent = this.Exponent,
                    InverseQ = this.InverseQ,
                    Modulus = this.Modulus,
                    P = this.P,
                    Q = this.Q

                };
            }
        }
    }
}
